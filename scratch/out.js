...
var formatErrorAsCard = (errorMsg, context = "Error") => {
  ...
  switch (statusCode) {
    case 400:
      title = "Bad Request";
      icon = "alert-circle";
      typeClass = " err-bad-request";
      break;
    case 401:
      title = "Unauthorized";
      icon = "key";
      typeClass = " err-unauthorized";
      break;
    case 403:
      title = "Forbidden";
      icon = "shield-alert";
      typeClass = " err-forbidden";
      break;
    case 404:
      title = "Not Found";
      icon = "search-x";
      typeClass = " err-not-found";
      break;
    case 408:
      title = "Request Timeout";
      icon = "timer";
      typeClass = " err-timeout";
      break;
    case 409:
      title = "Conflict";
      icon = "git-merge";
      typeClass = " err-conflict";
      break;
    case 413:
      title = "Payload Too Large";
      icon = "hard-drive";
      typeClass = " err-too-large";
      break;
    case 415:
      title = "Unsupported Media Type";
      icon = "file-question";
      typeClass = " err-unsupported";
      break;
    case 422:
      title = "Unprocessable Entity";
      icon = "file-x";
      typeClass = " err-unprocessable";
      break;
    case 429:
      title = "Rate Limit Exceeded";
      icon = "clock";
      typeClass = " err-rate-limit";
      break;
    case 500:
      title = "Internal Server Error";
      icon = "server-crash";
      typeClass = " err-server";
      break;
    case 502:
      title = "Bad Gateway";
      icon = "network";
...
var incognitoChatStore = /* @__PURE__ */ new Map();
...
function renderHome(container) {
    ...
    const renderDropdown = (listEl) => {
      ...
      noProjEl.addEventListener("click", () => {
        if (isIncognitoMode) {
          ...
          const newChats = [incognitoChat, ...(state2.chats || []).filter((c) => !c.incognito)];
  ...
  container.querySelector("#close-project-modal")?.addEventListener("click", () => {
  ...
  const renderChatHistory = () => {
    ...
    const activeChat = activeChatId ? state.activeProjectId ? state.projects.find((p) => p.id === state.activeProjectId)?.chats?.find((c) => c.id === activeChatId) : state.chats.find((c) => c.id === activeChatId) : null;
    if (chatHeader) {
      ...
      if (messages.length > 0) {
        messages.forEach((msg, i) => {
          ...
          if (msg.role === "assistant") {
            if (msg.tool_calls && msg.tool_calls.length > 0) {
              ...
              const getLabels = (tc, resultText) => {
                ...
                switch (tcName) {
                  case "read_file":
                    return [`Reading ${baseName(p.filepath) || "file"}...`, `Read ${baseName(p.filepath) || "file"}`];
                  case "write_file":
                    return [`Writing ${baseName(p.filepath) || "file"}...`, `Wrote ${baseName(p.filepath) || "file"}`];
                  case "list_files":
                    return [`Listing ${baseName(p.dirpath) || "directory"}...`, `Listed ${baseName(p.dirpath) || "directory"}`];
                  case "glob":
                  ...
                  case "tree":
                    return [`Generating tree for ${baseName(p.dirpath) || "directory"}...`, `Generated tree for ${baseName(p.dirpath) || "directory"}`];
                  case "search_files":
                    return [`Searching for "${p.query || ""}"...`, `Searched for "${p.query || ""}"`];
                  case "get_current_dir":
                    return [`Getting current path...`, `Got current path`];
                  case "date":
                    return [`Getting current date and time...`, `Got current date and time`];
                  case "path_stats":
                    return [`Getting stats for ${baseName(p.path) || "path"}...`, `Got stats for ${baseName(p.path) || "path"}`];
                  case "readlines":
                    return [`Reading lines from ${baseName(p.filepath) || "file"}...`, `Read lines from ${baseName(p.filepath) || "file"}`];
                  case "writelines":
                    return [`Writing lines to ${baseName(p.filepath) || "file"}...`, `Wrote lines to ${baseName(p.filepath) || "file"}`];
                  case "run_command":
                    return [`Running ${p.command || "command"}...`, `Ran ${p.command || "command"}`];
                  case "delete_path":
                    return [`Deleting ${baseName(p.path) || "path"}...`, `Deleted ${baseName(p.path) || "path"}`];
                  case "rename_path":
                    return [`Renaming ${baseName(p.old_path) || "path"}...`, `Renamed ${baseName(p.old_path) || "path"}`];
                  case "create_directory":
                    return [`Creating ${baseName(p.path) || "directory"}...`, `Created ${baseName(p.path) || "directory"}`];
                  case "search_web":
                    return [`Searching web for "${p.query || ""}"...`, `Searched web for "${p.query || ""}"`];
                  case "fetch_url":
                    return [`Fetching ${getHost(p.url)}...`, `Fetched ${getHost(p.url)}`];
                  case "next_search_batch":
                    return [`Reading next batch from ${getHost(p.url)}...`, `Read next batch from ${getHost(p.url)}`];
                  default:
                    return [`Executing ${tcName}...`, `Executed ${tcName}`];
                }
              };
              ...
              const items = (toolCalls.length > 0 ? toolCalls : [null]).map((tc, tcIdx) => {
                ...
                if (tc && getTcName(tc) === "edit_file") {
                  const changes = window._changeTracker?.getChanges ? window._changeTracker.getChanges() : changeTracker ? changeTracker.getChanges() : [];
                  const params = getTcParams(tc);
                  let targetPath = params.filepath || "";
                  return `<div style="padding:10px; color:orange; font-size:12px; font-family:monospace; border:1px solid orange; margin:8px 0;">
                                         DEBUG INFO:<br/>
                                         isCompleted: ${isCompleted}<br/>
                                         toolResultText: ${toolResultText ? "length=" + toolResultText.length + ", startsWithError=" + toolResultText.startsWith("Error") : "UNDEFINED"}<br/>
                                         targetPath: ${targetPath}<br/>
                                         changes array length: ${changes.length}<br/>
                                         found change: ${!!changes.find((c) => c.filepath.includes(targetPath) || targetPath.includes(c.filepath))}
                                     </div>`;
                }
                if (tc && getTcName(tc) === "edit_file" && isCompleted && toolResultText && !toolResultText.startsWith("Error")) {
                  const params = getTcParams(tc);
                  let targetPath = params.filepath || "";
                  const baseName2 = targetPath.split(/[/\\]/).pop();
                  const changes = window._changeTracker?.getChanges ? window._changeTracker.getChanges() : changeTracker.getChanges();
                  console.log("edit_file UI rendering, targetPath:", targetPath, "changes:", changes);
                  const change = changes.find((c) => c.filepath.includes(targetPath) || targetPath.includes(c.filepath));
                  console.log("Matched change:", change);
                  if (!change || !change.patch) {
                    return `<div style="color: red; padding: 10px;">DEBUG: edit_file failed to find patch. targetPath: "${targetPath}", changes: ${changes.length}, change found: ${!!change}</div>`;
                  }
                  if (change && change.patch) {
                                         ...
                                         <div class="search-web-container" data-msg-id="${msg.id}" data-tc-idx="${tcIdx}" style="margin: 8px 0;">
                                             <div class="search-web-summary" onclick="this.parentElement.classList.toggle('open')" style="display:flex;align-items:center;justify-content:space-between;cursor:pointer;color:var(--text-secondary);font-size:13px;padding:4px 0;user-select:none;">
                                                 <div style="display:flex;align-items:center;gap:8px;flex-grow:1;">
                                                     ...
                                                     <span style="color:var(--text-primary);">Edited <b>${baseName2}</b></span>
                                                 </div>
                                                 <div style="display:flex;align-items:center;gap:8px;">
                  ...
                  if (fileTcs.length > 0) {
                    filesChanged = fileTcs.map((tc) => {
                      ...
                      if (tcName.includes("write")) {
                        let contentStr = "";
                        if (p.content !== void 0) contentStr = String(p.content);
                        else if (p.CodeContent !== void 0) contentStr = String(p.CodeContent);
                        ...
                        let contentStr = "";
                        if (p.content !== void 0) contentStr = String(p.content);
                        else if (p.ReplacementContent !== void 0) contentStr = String(p.ReplacementContent);
                        additions = contentStr === "" ? 0 : contentStr.split(/\r?\n|\n/).length;
            ...
            setTimeout(() => {
              const wrapper2 = row.querySelector(".intent-chip-wrapper");
              const popup = row.querySelector(".intent-popup");
              const chevron = row.querySelector('[data-lucide="chevron-right"]');
              if (wrapper2 && popup && chevron) {
                const toggleShow = (force) => {
                  const isShowing = force !== void 0 ? force : popup.style.display === "flex";
                  popup.style.display = isShowing ? "flex" : "none";
                  chevron.style.transform = isShowing ? "rotate(90deg)" : "rotate(0deg)";
                };
                wrapper2.addEventListener("mouseenter", () => toggleShow(true));
                wrapper2.addEventListener("mouseleave", () => toggleShow(false));
                wrapper2.addEventListener("click", () => toggleShow(popup.style.display !== "flex"));
              }
            }, 0);
          ...
          if (!row.classList.contains("tool-call-row") || msg.role === "user") {
            ...
            if (msg.stats) {
              const timeDisplay = msg.stats.timeMs > 1e3 ? (msg.stats.timeMs / 1e3).toFixed(1) + "s" : msg.stats.timeMs + "ms";
            ...
            actionBtns.querySelectorAll(".edit-msg-btn").forEach((btn) => {
              btn.addEventListener("click", () => {
                const promptBox2 = document.getElementById("promptBox");
                if (!promptBox2) return;
                promptBox2.value = msg.content;
                promptBox2.style.height = "auto";
                promptBox2.style.height = promptBox2.scrollHeight + "px";
                promptBox2.focus();
  ...
  const processAIResponse = async (responseObj) => {
    ...
    if (toolCalls.length > 0) {
      ...
      toolCalls.forEach((tc, i) => {
        ...
        console.log(`Tool Result for ${tcName}:`, results[i]);
        stateManager.addMessage("tool", results[i], [], null, { tool_call_id: tc.id, name: tcName });
      });
      if (!stateManager.isGenerating) return;
  ...
  const submitPrompt = async () => {
    ...
    try {
      ...
      const state2 = stateManager.getState();
      if (state2.intentAnalyzerModel) {
        console.log(`Analyzing intent for: "${text}" using model: ${state2.intentAnalyzerModel}`);
        ...
        try {
          intentResult = await llmService.analyzeIntent(text, state2.intentAnalyzerModel, stateManager.getCancelSignal());
        } catch (err) {
  ...
  const unsubscribeChat = stateManager.subscribe((state) => {
    ...
    renderProjects();
    renderRecentConvos();
    const sendBtn2 = document.getElementById("send-prompt-btn");
    if (sendBtn2) {
      if (stateManager.isGenerating) {
        sendBtn2.innerHTML = '<i data-lucide="square" class="icon-svg sm" style="color:var(--error-color);"></i>';
        sendBtn2.classList.add("is-generating");
      } else {
        sendBtn2.innerHTML = '<i data-lucide="arrow-right" class="icon-svg sm"></i>';
        sendBtn2.classList.remove("is-generating");
      }
      if (window.lucide) window.lucide.createIcons({ root: sendBtn2 });
    }
